...
function itsLoaded() {
  //this function gets called once page is loaded
...
function initialSubmit() {
  //this is the button that submits player amount and choices per
  ...
  if (homeCleansing(currentGame.playerCount, currentGame.placesPerPlayer)) {
    ...
    nameForm.innerHTML +=
      '<input type="button" id=playerBtn value="SUBMIT" onclick="nameSubmit();"/>';
...
function nameSubmit() {
  ...
  for (let i = 1; i <= currentGame.playerCount; i++) {
    ...
    nameP.innerHTML =
      i + ". " + document.getElementById("nameBox" + i).value + " <br>";
    nameP.id = "nameP" + i;
  ...
  placeForm.innerHTML =
    currentGame.names[0] +
    "'s turn: " +
    '<input type="text" id="placeInput"/> <br>';
  placeForm.innerHTML +=
    '<br><input type="button" value="SUBMIT" id="placeBtn" onclick="placeSubmit();"/>';
...
function placeSubmit() {
  ...
  //set placePrompt & placeBtn
  placeForm.innerHTML =
    currentGame.names[currentGame.placeCounter % currentGame.playerCount] +
    "'s turn: " +
    '<input type="text" id="placeInput"/> <br>';
  placeForm.innerHTML +=
    '<br><input type="button" value="SUBMIT" id="placeBtn" onclick="placeSubmit();"/>';
  ...
  if (
    currentGame.placeCounter >
    currentGame.placesPerPlayer * currentGame.playerCount
  ) {
    ...
    for (
      ...
      var newButton =
        '<input type="image" id = trash' +
        currentGame.placeCounter +
        ' alt="trash" src="pictures/trash.png" height=19.6px width=13.4px onclick="trashButton(' +
        currentGame.placeCounter +
        ');">';
      //changed it to input because no one uses just buttons hehe

      document.getElementById(
        "place" + currentGame.placeCounter
      ).innerHTML += newButton; // adds button to line
    }
...
function setBanWording(newBanner) {
  for (let i = 1; i <= currentGame.playerCount; i++) {
    if (i == newBanner) {
      document.getElementById("nameP" + i).innerHTML =
        "<B>" + i + ". " + currentGame.names[i - 1] + "</B>";
      document.getElementById("nameP" + i).style.color = "black";
    } else {
      document.getElementById("nameP" + i).innerHTML =
        i + ". " + currentGame.names[i - 1];
      document.getElementById("nameP" + i).style.color = "white";
    }
  }
}
...
function trashButton(placeCount) {
  ...
  if (
    currentGame.banAmount <
    currentGame.placesPerPlayer * currentGame.playerCount - 1
  ) {
    document.getElementById("trash" + placeCount).style.display = "none";

    document.getElementById("place" + placeCount).innerHTML =
      "<s>" + document.getElementById("place" + placeCount).innerHTML + "<s>";
    ...
    for (
      let i = 1;
      i <= currentGame.placesPerPlayer * currentGame.playerCount;
      i++
    ) {
      if (document.getElementById("trash" + i).style.display != "none") {
        //we have found winner!
        document.getElementById("place" + i).style.color = "black";
        finalWinner = i;
    ...
    winText.innerHTML =
      "<div class='row'>The winning place is <b>" +
      currentGame.places[finalWinner - 1] +
      "</b>!</div>";
...
function setWinnerWording(newWinner) {
  if (newWinner == 0) {
    ...
    document.getElementById("nameP" + newWinner).innerHTML =
      "<B>" + newWinner + ". " + currentGame.names[newWinner - 1] + "</B>";
    document.getElementById("nameP" + newWinner).style.color = "black";
  }
}
